...
static char *disk_io(const char *dev);
static char *disk_perc(const char *mnt);
static char *disk_total(const char *mnt);
static char *disk_used(const char *mnt);
static char *disk(double *dw_old, double *dr_old, const char* dev);
static char *entropy(void);
static char *fan_ibm(const char* path);
static char *fan_framework(void);
...
weather()
{
    if(weatherInit) {
        weatherInit = 0;
        weatherTemp = run_command("weather");
        RETURN_FORMAT(7, "%s", weatherTemp);
    }
    if(time(NULL) % 3600 != 0) {
        RETURN_FORMAT(7, "%s", weatherTemp);
    }
...
battery_time(const char *bat)
{
    ...
    // 1. Get Status (Charging/Discharging)
    snprintf(path, sizeof(path), "/sys/class/power_supply/%s/status", bat);
    if (!(fp = fopen(path, "r"))) return UNKNOWN_STR;
    ...
    if (!(fp = fopen(path, "r"))) return UNKNOWN_STR;
    fscanf(fp, "%ld", &current);
    fclose(fp);

    // Avoid division by zero if idle
    if (current <= 0) return UNKNOWN_STR;

    // 3. Calculate time
    if (strcmp(status, "Discharging") == 0) {
        time_mins = (int)((double)charge / current * 60);
    } else if (strcmp(status, "Charging") == 0) {
        // You'd need charge_full to calculate time-to-full
        long full;
        snprintf(path, sizeof(path), "/sys/class/power_supply/%s/charge_full", bat);
        if (!(fp = fopen(path, "r"))) return UNKNOWN_STR;
        fscanf(fp, "%ld", &full);
        fclose(fp);
        time_mins = (int)((double)(full - charge) / current * 60);
    } else {
        return UNKNOWN_STR;
    }
...
cpu_perc(long double ps_old[4])
{
    ...
    fscanf(fp, "%*s %Lf %Lf %Lf %Lf", &ps[0], &ps[1], &ps[2], &ps[3]);
    fclose(fp);
...
fan_framework(void)
{
    ...
    // Return the higher of the two, or format however you prefer
    int max_fan = (fan1 > fan2) ? fan1 : fan2;
    RETURN_FORMAT(9, "%04d", max_fan);
}

static char *
fan_ibm(const char* path)
{
    int fan;
    FILE *fp;
...
disk_free(const char *mnt)
{
    ...
    RETURN_FORMAT(10, "%.1f", (float)fs.f_bsize * (float)fs.f_bfree / 1024 / 1024 / 1024);
}
...
disk_total(const char *mnt)
{
    ...
    RETURN_FORMAT(10, "%.1f", (float)fs.f_bsize * (float)fs.f_blocks / 1024 / 1024 / 1024);
}
...
disk_used(const char *mnt)
{
    ...
    RETURN_FORMAT(10, "%.1f", (float)fs.f_bsize * ((float)fs.f_blocks - (float)fs.f_bfree) / 1024 / 1024 / 1024);
}

static char *
disk_io(const char *dev)
{
    int diskIO;
    FILE *fp;
    ...
    while (fgets(line, sizeof(line), fp)) {
        if (strstr(line, base)) {
            sscanf(line, "%*d %*d %*s %*d %*d %*d %*d %*d %*d %d", &diskIO);
            break;
        }
    }
    fclose(fp);
...
load_avg(void)
{
    ...
    int perc = 100 * avgs[0]/get_nprocs();

    RETURN_FORMAT(50, "%*d", 3, perc);
}
...
ram_total(void)
{
    ...
    RETURN_FORMAT(10, "%d", (int)total / 1024);
}

static char *
ram_used(void)
{
    long free, total, buffers, cached, arc;
    FILE *fp;
    ...
    // fp = fopen("/proc/spl/kstat/zfs/arcstats", "r");
    // if (fp == NULL) {
    //     warn("Failed to open file /proc/spl/kstat/zfs/arcstats");
    //     RETURN_FORMAT(10, UNKNOWN_STR);
    // }
    //
    // char line[256];
    ...
    //         sscanf(line, "size 4 %ld", &arc);
    //         break;
    //     }
    // }
    // arc = arc / 1024 / 1024;

    // RETURN_FORMAT(10, "%d", (int)(total - free - buffers - cached) / 1024 - (int)arc);
    RETURN_FORMAT(10, "%d", (int)(total - free - buffers - cached) / 1024);
}
...
static char *
disk(double *dw_old, double *dr_old, const char* dev)
{
    double dw_val, dw_buf = 0;
    double dr_val, dr_buf = 0;
    const char *base = strrchr(dev, '/');
    base = base ? base + 1 : dev;

    int sector_size = get_sector_size(base);
    if (sector_size < 0) {
        warn("get_sector_size");
        RETURN_FORMAT(10, UNKNOWN_STR);
    }
    ...
    while (fgets(line, sizeof(line), f)) {
        if (strstr(line, base)) {
            sscanf(line, "%*d %*d %*s %*f %*f %lf %*f %*f %*f %lf",
                   &dr_buf, &dw_buf);
            break;
        }
    }
    ...
    dw_val = dw_buf-*dw_old;
    *dw_old = dw_buf;
    dr_val = dr_buf-*dr_old;
    *dr_old = dr_buf;

    if (dw_val < 1000) {
        RETURN_FORMAT(30, "[%-4.3g B/s : %-4.3g B/s]", dr_val, dw_val);
    } else if (dw_val < 1024000) {
        RETURN_FORMAT(30, "[%-4.3gKB/s : %-4.3gKB/s]", dr_val/1024.0, dw_val/1024.0);
    } else {
        RETURN_FORMAT(30, "[%-4.3gMB/s : %-4.3gMB/s]", dr_val/1048576.0, dw_val/1048576.0);
    }
}
...
#define disk(dev)           disk(&dw_old, &dr_old, dev)
    long double ps_old[4];
    double rx_old, tx_old;
    double dr_old, dw_old;
